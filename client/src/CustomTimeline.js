          ...
          <div className="timeline-grid" ref={gridRef}>
            ...
            <div className="grid-overlay">
              {dateColumns.map((date, i) => {
                ...
                const monthBg = monthIndex % 2 === 0 ? 'rgba(102, 126, 234, 0.02)' : 'rgba(52, 168, 83, 0.02)';
                  ...
                  if (currentMonth !== monthKey) {
                    if (currentMonth !== null) {
                      monthSpans.push({ 
                        start: spanStart, 
                        count: spanCount, 
                        date: dateColumns[spanStart],
                        monthIndex: monthSpans.length 
                      });
                    }
                ...
                return monthSpans.map((span, idx) => (
                  <div 
                    ...
                    style={{
                      ...
                      backgroundColor: span.monthIndex % 2 === 0 ? 'rgba(102, 126, 234, 0.08)' : 'rgba(52, 168, 83, 0.08)',
                      borderBottom: '1px solid #e2e8f0',
                      padding: '4px 8px',
                      textAlign: 'center',
                      fontSize: '12px',
                  ...
                  >
                    {span.date.toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                  </div>
                ));
              })()}
            </div>
            ...
            <div className="timeline-header-row">
              {dateColumns.map((date, i) => {
                ...
                // Determine month for alternating backgrounds
                const monthIndex = Math.floor((date - new Date(date.getFullYear(), 0, 1)) / (1000 * 60 * 60 * 24 * 30));
                const monthBg = monthIndex % 2 === 0 ? 'rgba(102, 126, 234, 0.03)' : 'rgba(52, 168, 83, 0.03)';
                
                return (
                  <div 
                    key={i} 
                    className={`timeline-date-header ${isWeekend ? 'weekend' : ''}`}
                    style={{ backgroundColor: isWeekend ? undefined : monthBg }}
                  >
